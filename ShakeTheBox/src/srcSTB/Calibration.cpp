...
//	}
	...
	//for (int h = 0; h < nCams; ++h) cleanlist->reserve(1000000);
	//std::cout << "capacity = " << cleanlist[0].capacity() << "\n";
	//_getch();
...
//		}
		...
		//cout << "i = " << i << " of " << corrframes[rID[0]].NumParticles()  << ": tmplist0on1.size() = " << tmplist0on1.size() << endl;
		//_getch();
//#pragma omp for schedule(guided)
		...
		for (int it = 0; it < tmplist0on1.size(); it++) {
			...
			//vector<Frame::const_iterator> tmplist0n1on2_;
			...
			ParticleFinder2to1_(camID[2], rID[2], camID[0], camID[1], pAworld, pBworld, mindist_1D, tmplist0n1on2, *pA, *pB);

			//if (!tmplist0n1on2.empty())
			//	cout << "size = " << tmplist0n1on2.size() << "\n";

			//if (!tmplist0n1on2_.empty())
			//	cout << "size_ = " << tmplist0n1on2_.size() << "\n";
			...
			for (int itt = 0; itt < tmplist0n1on2.size(); itt++) {
				...
				PosToMatch.push_back(*pA);
				PosToMatch.push_back(*pB);
				PosToMatch.push_back(*pC);
#endif
				...
				if ( 
#ifdef MEM_OPT 
					checkMindist3DforMemOpt(PosToMatch) &&
#endif
					ParticleCheck2to1(camID[0], camID[1], camID[2], pBworld, pCworld, pA, mindist_1D) && ParticleCheck2to1(camID[1], camID[0], camID[2], pAworld, pCworld, pB, mindist_1D)
					) {
					if (rcams == 3) {
						try {
#pragma omp critical(cleanlist) // to indicate this sentence should only be executed by one thread at a time
					...
					//vector<Frame::const_iterator> tmplist1n2on3_;
					...
					ParticleFinder2to1_(camID[3], rID[3], camID[1], camID[2], pBworld, pCworld, mindist_1D, tmplist1n2on3, *pB, *pC);
...
#ifdef MEM_OPT
						deque<Position> PosToMatch2;
						PosToMatch2.push_back(*pA);
						PosToMatch2.push_back(*pB);
						PosToMatch2.push_back(*pC);
						PosToMatch2.push_back(*pD);
#endif

						// checking if the potential candidates on 4th cam are within mindist_2D to a line proj from 1st cam
						if ( 
#ifdef MEM_OPT 
							checkMindist3DforMemOpt(PosToMatch2) &&
#endif
							ParticleCheck1to1(camID[0], camID[3], pAworld, pD, mindist_1D)
							) {
...
bool Calibration::checkMindist3DforMemOpt(deque<Position> ipos)
{
	/*if (ipos.size() != cams.size()) {
		throw runtime_error("Number of cameras and number of images do not match!");o
	}*/
	...
	//std::cout << "test dist = " << dist << "\n";

	return (dist < mindist_3D * mindist_3D) ? true : false;
}
...
//	if (mindist > 1.5)
//		mindist = 1.5;
	double mindist_sin = abs(mindist_1D / sin(PI * angle / 360))/5;
	double mindist_cos = abs(mindist_1D / cos(PI * angle / 360));
	double mindist = max(mindist_sin, mindist_cos);
	/*
	 * Modified by Shiyong Tan
	 * Date: 6.7.18
	...
	Position p1(x - mindist_sin, y - mindist_cos, 0);
	Position p2(x + mindist_sin, y + mindist_cos, 0);
	Position pPix0(cams[camID].Distort(p));
	...
	//std::cout << xpix1 << " " << xpix2 << " " << ypix2 << " " << ypix1 << "\n";
	...
	{
		double mul1 = 1.0/sqrt(1 + pow(m1, 2));
		double mul2 = 1.0/sqrt(1 + pow(m2, 2));
		xpix1 = max(0, xpix1);
		ypix2 = max(0, ypix2);
		...
		for (int xpix = xpix1; xpix <= xpix2; xpix++)	
			for (int ypix = ypix2; ypix <= ypix1; ypix++)	
				if (is_Particle[rID][ypix][xpix] == 1) {
					auto &tmp = is_ParticlePos[rID][ypix][xpix];
					int size = tmp.size();
					for (int it = 0; it < size; it++) {
						Frame::const_iterator pX = tmp[it];
						Position dist(*pX - p);
						if (dist.Magnitude() < mindist) {						// if the distance of the particle from intersection point is smaller than midist and
							double dist1 = abs(-m1 * pX->X() + pX->Y() - c1) * mul1;	// distance of particle from line1 
							double dist2 = abs(-m2 * pX->X() + pX->Y() - c2) * mul2;	// distance of particle from line2
							if (dist1 < mindist_1D && dist2 < mindist_1D) {	// if the particle is within mindist_1D dist from both the lines,
//cout << mindist_1D << "," << mindist << "," << angle << "," << sin(PI*angle/360) << " : " << pPix0.X() - xpix << " " << pPix0.Y() - ypix << " : " << pPix0.X() << " " << pPix0.Y() << " : " << theta1 << " " << theta2 << " : " << xpix2 - xpix1 << " " << ypix1 - ypix2 << endl;
...
// A function that creates a search area around the intersection of 2 projected lines (from camID 1 and camid2) on camID
// and pushes all the particles within this search area to tmplist
void Calibration::ParticleFinder2to1_(int camID_, int rID, int camid1, int camid2, Position P1world, Position P2world, double mindist_1D, vector<Frame::const_iterator>& tmplist, Position P1img, Position P2img)
{
	/*if (ipos.size() != cams.size()) {
	throw runtime_error("Number of cameras and number of images do not match!");o
}*/
...
// http://en.wikipedia.org/wiki/Line-line_intersection
	...
	std::deque<Position> ipos;
	ipos.push_back(P1img);
	ipos.push_back(P2img);
	int size = ipos.size();

	std::vector<int> myCamIDs;
	myCamIDs.push_back(camid1);
	myCamIDs.push_back(camid2);

	for (int i = 0; i < size; ++i) {
		// construct a line of sight for this point:
		// vector from camera center to this point (in world coordinates)
		sight[i] = cams[myCamIDs[i]].ImageToWorld(ipos[i]) - cams[myCamIDs[i]].Center();
		// normalize
		sight[i] /= sight[i].Magnitude();
	...
	Position p_(cams[camID_].WorldToImage(worldpos));
	double x = p_.X();
	double y = p_.Y();

	double mindist = mindist_1D;

	Position p(x, y, 0);
	// forming a search area around the intersection pointExtra
	Position p1(x - mindist, y - mindist, 0);
	Position p2(x + mindist, y + mindist, 0);
	Position pPix0(cams[camID_].Distort(p));
	Position pPix1(cams[camID_].Distort(p1));
	Position pPix2(cams[camID_].Distort(p2));
	// bounds of search area in pixels_orig
	int xpix1 = floor(pPix1.X());
	int xpix2 = ceil(pPix2.X());
	int ypix1 = ceil(pPix1.Y());
...
// projecting the lines from camid1, camid2 onto camID and finding their intersection (x,y)
	Position center1(cams[camID_].WorldToImage(cams[camid1].Center()));
	Position particle1(cams[camID_].WorldToImage(P1world));
	double m1 = (center1.Y() - particle1.Y()) / (center1.X() - particle1.X());
	double c1 = particle1.Y() - m1 * particle1.X();
	Position center2(cams[camID_].WorldToImage(cams[camid2].Center()));
	Position particle2(cams[camID_].WorldToImage(P2world));
	double m2 = (center2.Y() - particle2.Y()) / (center2.X() - particle2.X());
	double c2 = particle2.Y() - m2 * particle2.X();

	int Npixw = cams[camID_].Get_Npixw();
	int Npixh = cams[camID_].Get_Npixh();
